...
class PhiForCausalLM(PhiPreTrainedModel):
    ...
    def forward(
        ...
        logits = self.lm_head(hidden_states)  # 一层linear [hidden_size --> vocab_size]
        logits = logits.float()

        loss = None
        if labels is not None:
            # Shift so that tokens < n predict n
            # 这行代码对模型的预测结果（logits）进行操作。在自回归模型中，我们希望使用前一个词来预测下一个词
            # 因此，这里移除了每个序列最后一个词的预测（因为它没有下一个词可以预测）
            # contiguous()的作用是确保tensor在内存中是连续的，这通常在reshape操作之前需要保证
            shift_logits = logits[..., :-1, :].contiguous()
            shift_labels = labels[..., 1:].contiguous()  # 将序列向左移动一位，预测的第一个词应当使用标签中的第二个词作为真实标签。
            # Flatten the tokens
